...
if [[ -n ${self_hostname} ]]; then
  sed -i "s/self_hostname: localhost/self_hostname: $self_hostname/g" "$SIT_ROOT/config/config.yaml"
fi

# TODO: Document why this is needed
sed -i 's/localhost/127.0.0.1/g' "$SIT_ROOT/config/config.yaml"

if [[ ${log_to_file} != 'true' ]]; then
  sed -i 's/log_stdout: false/log_stdout: true/g' "$SIT_ROOT/config/config.yaml"
else
  sed -i 's/log_stdout: true/log_stdout: false/g' "$SIT_ROOT/config/config.yaml"
fi
