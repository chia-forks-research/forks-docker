...
if [[ -n ${crawler_db_path} ]]; then
  chives configure --crawler-db-path "${crawler_db_path}"
fi

if [[ -n ${crawler_minimum_version_count} ]]; then
  chives configure --crawler-minimum-version-count "${crawler_minimum_version_count}"
fi

if [[ -n ${self_hostname} ]]; then
  sed -i "s/self_hostname: localhost/self_hostname: $self_hostname/g" "$CHIVES_ROOT/config/config.yaml"
fi
